...
def convert_time_only_indexedvar_to_cuid(var, t_set):
    from pyomo.core.base.var import IndexedVar
    if not type(var) is IndexedVar:
        raise RuntimeError("Given var: ", var.name,
                           "is not a pyomo IndexedVar")
    if var.index_set() is not t_set:
        raise RuntimeError(var.name,
                           "is not or not only indexed by the time set.")
